...
trait Tables {
  ...
  class Authors(tag: Tag) extends Table[AuthorsRow](tag, "authors") {
    ...
    def ? = (id.?, email.?).shaped.<>({r=>import r._; _1.map(_=> AuthorsRow.tupled((_1.get, _2.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  implicit def GetResultAuthorscontributionRow(implicit e0: GR[Int], e1: GR[String]): GR[AuthorscontributionRow] = GR{
    prs => import prs._
    AuthorscontributionRow.tupled((<<[Int], <<[String], <<[String], <<[String], <<[Int], <<[Int]))
  }
  ...
  class Authorscontribution(tag: Tag) extends Table[AuthorscontributionRow](tag, "AuthorsContribution") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (id.?, authorname.?, affiliation.?, contribution.?, percent.?, copyrighttransferid.?).shaped.<>({r=>import r._; _1.map(_=> AuthorscontributionRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  implicit def GetResultInternalUsersRow(implicit e0: GR[Int], e1: GR[String]): GR[InternalUsersRow] = GR{
    prs => import prs._
    InternalUsersRow.tupled((<<[Int], <<[String], <<[String]))
  }
  ...
  class InternalUsers(tag: Tag) extends Table[InternalUsersRow](tag, "INTERNAL_USERS") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (id.?, name.?, passwordSha1Sum.?).shaped.<>({r=>import r._; _1.map(_=> InternalUsersRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
